...
0.3.0 的定位是已登录终端的连接层，不是第二套 AI 权限系统。MCP 客户端决定是否批准命令；本工具负责明确目标、一次发送、输出与状态。不要把模型对风险的判断误认为 MCP 已经验证了用户批准。

## 推荐路径

先调用 `connector_list`，选择并核实目标。后续常规排查优先调用：
...
`max_bytes` 是第一段输出大小（4..65536，默认16384），不是远端日志全量大小。后续页使用 `connector_read` 和返回的 `next_cursor`。不要按字符数自己计算游标。缓存默认每命令1MiB，最多32份结果；满时淘汰最旧已结束结果。操作去重记录不会随输出一起删除。
