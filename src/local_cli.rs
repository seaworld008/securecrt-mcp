...
use crate::{audit::AuditLog, bridge::BridgeClient, config::{self, Config}, execution::Engine,
    fault, model::RunParams, policy::{Decision, PolicyEngine}};
...
fn input(path: &str) -> Result<Value> {
    ...
    ensure!(bytes.len() <= 262_144, "invalid input: JSON exceeds 256 KiB");
...
pub async fn screen(session: String) -> Result<()> {
    emit(&engine()?.bridge.call("read_screen", json!({"session":session})).await?)
}
pub fn policy_check(path: &str) -> Result<()> {
    let value = input(path)?;
    let command = value["command"].as_str().ok_or_else(|| anyhow::anyhow!("invalid command"))?;
    let config = Config::load()?;
    let decision = PolicyEngine::new(&config.policy)?.classify_command(command);
    emit(&json!({"mode":config.policy.mode, "config":config::config_path()?,
...
pub async fn run(path: &str) -> Result<()> {
    let operation = async {
        ...
        while matches!(result["state"].as_str(), Some("starting" | "running")) {
            let id = result["command_id"].as_str().ok_or_else(|| anyhow::anyhow!("missing command_id"))?.to_owned();
    ...
    emit(&result)?;
    ensure!(result["state"] == "completed" && result["exit_code"].as_i64().is_none_or(|code| code == 0),
