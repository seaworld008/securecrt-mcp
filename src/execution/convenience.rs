...
impl Engine {
    ...
    async fn run_command_inner(&self, p: RunParams) -> Result<Value> {
        let timeout = p.timeout_ms.unwrap_or(30_000.min(self.config.bridge.max_command_timeout_ms));
        let max = p.max_bytes.unwrap_or(16_384);
        let wait = p.wait_ms.unwrap_or((timeout + 2 * self.config.bridge.request_timeout_ms).min(50_000));
        ...
        let op = p.operation_id.clone().unwrap_or_else(|| Uuid::new_v4().to_string());
        // Do not include ephemeral screen tokens or response pagination/wait choices.
        let fingerprint = format!("run:{:x}", Sha256::digest(serde_json::to_vec(&json!({
        ...
        let gate = self.run_gate.try_lock().map_err(|_| anyhow::anyhow!("busy: another command is preparing; nothing sent"))?;
        let existing = {
            ...
            if let Some((id, hash)) = registry.operations.get(&op) {
                ...
                ensure!(registry.busy.is_none(), "busy/unresolved: inspect current operation; nothing sent");
                None
            }
        };
        ...
        let screen = self.bridge.call("read_screen", json!({"session": p.session})).await
        ...
        let expected = match p.expected_prompt {
            Some(prompt) => {
                ensure!(prompt.trim_end() == current, "prompt_mismatch: expected input changed; nothing sent");
                prompt.trim_end().to_owned()
            }
            None => {
                ensure!(p.mode == CaptureMode::Posix && shell_prompt_candidate(current),
        ...
        let token = screen["screen_token"].as_str().ok_or_else(|| anyhow::anyhow!("bridge_incompatible: missing screen token"))?.to_owned();
        let job = self.submit_internal(ExecuteParams {
            session: p.session, screen_token: token, expected_prompt: expected,
            operation_id: op, command: p.command, mode: p.mode,
            wait_for: p.wait_for, timeout_ms: Some(timeout), settle_ms: p.settle_ms,
        }, Some(fingerprint)).await?;
        let id = job["command_id"].as_str().ok_or_else(|| anyhow::anyhow!("missing command_id"))?.to_owned();
        drop(gate);
    ...
    pub async fn wait_result(&self, id: &str, wait_ms: u64, max_bytes: usize) -> Result<Value> {
        ensure!(wait_ms <= 60_000 && (4..=65_536).contains(&max_bytes), "invalid bounded wait/output parameters");
        let until = Instant::now() + Duration::from_millis(wait_ms);
        loop {
            let registry = self.inner.lock().await;
            let job = registry.jobs.get(id).ok_or_else(|| fault::BridgeFault {
                message: "unknown/expired command_id; never infer it was not executed".into(), sent: None,
            })?;
            if !job.state.active() || Instant::now() >= until {
                ...
                result["next_cursor"] = if end < job.output.len() || job.state.active() { json!(end) } else { Value::Null };
...
fn shell_prompt_candidate(line: &str) -> bool {
    if line.is_empty() || line.len() > 512 || line.chars().any(char::is_control) { return false; }
    let lower = line.to_lowercase();
    if ["password", "passphrase", "--more--", "verification code", "otp:", "密码"].iter().any(|s| lower.contains(s)) { return false; }
...
mod tests {
    ...
    fn prompt_candidate_does_not_guess_password_or_continuation_context() {
        ...
        for line in [">", ">>>", "mysql>", "Password:", "--More--", "", "Password#"] {
