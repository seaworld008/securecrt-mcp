...
impl AuditLog {
    ...
    pub fn with_sync(mut self, sync_each: bool) -> Self {
...
mod tests {
    ...
    async fn missing_directory_is_not_silently_ignored() {
        let path = std::env::temp_dir()
            ...
            .join("audit.jsonl");
        assert!(
            AuditLog::new(true, false, path)
                .record("dispatch", "id", "s", "test", None)
                .await
                .is_err()
        );
    }
    ...
    async fn buffered_and_durable_writes_are_visible_before_return() {
        for sync in [false, true] {
            let path = std::env::temp_dir()
                .join(format!("securecrt-audit-{}.jsonl", uuid::Uuid::new_v4()));
            let log = AuditLog::new(true, false, path.clone()).with_sync(sync);
            for i in 0..3 {
                log.record("test", &i.to_string(), "s", "test", Some("sensitive text"))
                    .await
                    .unwrap();
            }
